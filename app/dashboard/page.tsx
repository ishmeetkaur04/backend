    ...
    <div className="flex min-h-screen bg-gray-100">
      ...
      <div className="w-64 bg-white shadow-md p-6">
        ...
        <div className="flex flex-col gap-6">
          <Link href="/dashboard" className="flex items-center gap-3 hover:text-blue-600">
            <LayoutDashboard size={20} />
            Dashboard
          </Link>

          <Link href="/send" className="flex items-center gap-3 hover:text-blue-600">
            <Send size={20} />
            Send Money
          </Link>

          <Link href="/transactions" className="flex items-center gap-3 hover:text-blue-600">
            <ArrowDownLeft size={20} />
            Transactions
          </Link>

          <Link href="/cards" className="flex items-center gap-3 hover:text-blue-600">
            <CreditCard size={20} />
            Cards
          </Link>
        </div>
          ...
          {transactions.length === 0 ? (
            ...
            transactions.map((t: any, i) => (
              <div key={i} className="flex justify-between border-b py-3">
                <p>{t.user || t.name}</p>
                <p className="text-red-500">-₹{t.amount}</p>
              </div>
            ))
          )}
        </div>
